...
| Custom SVG result presentation | authored web visuals | awkward | The web renderer recognizes a returned `<svg...>` string, but the editor runner first echoes the entire source expression. Large inline literals therefore appear as raw SVG before the image; escaped hex colors can also confuse inline-comment presentation. Assignment binds but does not render the payload. A dedicated raw-SVG display primitive, or compact/collapsible source echo for visual results, is needed. A named docstring-first constructor, a variable assignment, one bare-variable display, and `rgb(...)` colors are the local workaround. |
